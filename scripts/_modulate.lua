...
function factory () return function ()
	...
	function dsp_modulate(ctrl, bufs, n_samples, offset, start)
		cnt = cnt or 0
		cnt = cnt + n_samples
		if cnt > 48000 then cnt = cnt - 48000 end
		local gain = ctrl:at (Evoral.Parameter (ARDOUR.AutomationType.PluginAutomation, 0, 3)):to_automationcontrol():to_plugincontrol()
		assert (gain)
		local a = ARDOUR.DSP.fast_coefficient_to_dB (ARDOUR.DSP.compute_peak(bufs:get_audio(0):data(offset), n_samples, 0))
		a = math.min (20, math.max(-20, a))
		gain:modulate_to (5 * math.sin (math.pi * cnt / 48000) - a)
	end
	]]
	))
